...
## Purpose
How one customer's financial data is kept out of another customer's report.

Tenancy is `organizations` -> `entities` -> `accounts`, and the predicate that separates
them lives in the database as a row-level-security policy, not in application code. The
distinction this capability turns on: row-level security is *containment*, not
authorization. It guarantees that a transaction bound to an organisation touches only that
organisation's rows; it cannot know whether the caller was entitled to bind to it. So the
tenant identifier is established through named constructors rather than asserted from a
request, and the two mechanisms are specified separately.

Created by archiving change add-tenancy-and-rls.
