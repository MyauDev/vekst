...
- [ ] 4.8 Document in the worker's package comment that a handler takes its tenant identifier from job arguments and never from ambient state, and why
...
- [ ] 7.3 Migration Job in the `local` overlay running `goose up` as `vekst_migrator` before `core` starts, ordered by a Tilt resource dependency
...
- [ ] 9.4 Update the capability spec and run the full suite
