...
COMPANIES = [
    "ГРАНИТ ПЛЮС", "ВЕКТОР ТРЕЙД", "СЕВЕРНЫЙ ПУТЬ", "АЛЬФА СИСТЕМС", "БЕЛАЯ РЕКА",
    "ТЕХНОПАРК СИТИ", "МЕРИДИАН ГРУПП", "ЯНТАРЬ ЛОГИСТИК", "ЗЕЛЁНЫЙ ОСТРОВ",
    "ПЕРВАЯ СТУДИЯ", "КАСКАД ИНЖИНИРИНГ", "ОРИОН МЕДИА", "ДЕЛЬТА СЕРВИС",
    "СТРЕЛА КОНСАЛТ", "НОВЫЙ ФОРМАТ", "ПОЛЯРНАЯ ЗВЕЗДА", "КАМЕННЫЙ МОСТ",
    "ЛИНИЯ РОСТА", "СИНИЙ КИТ", "ТРИ СОСНЫ",
]
LATIN_COMPANIES = [
    "NORTHWIND TRADING", "BLUEPINE SYSTEMS", "CEDAR POINT LLC", "HARBOUR SOFT",
    "MERIDIAN LABS", "STONEBRIDGE INC", "AMBERWAY GROUP", "FIRSTLIGHT OU",
    "GREENFIELD SP", "SILVERLAKE AB", "OAKROW LIMITED", "REDCLIFF CORP",
]
SURNAMES = ["ИВАНОВ", "ПЕТРОВ", "СИДОРОВ", "КОЗЛОВ", "НОВИКОВ", "МОРОЗОВ",
            "ВОЛКОВ", "СОКОЛОВ", "ЛЕБЕДЕВ", "ЗАЙЦЕВ"]
GIVEN = ["АЛЕКСЕЙ", "МАРИЯ", "ДМИТРИЙ", "ЕЛЕНА", "СЕРГЕЙ", "ОЛЬГА", "АНДРЕЙ", "ИРИНА"]
PATRONYMIC = ["ВИКТОРОВИЧ", "ПЕТРОВНА", "СЕРГЕЕВИЧ", "ИВАНОВНА", "АНДРЕЕВИЧ", "ОЛЕГОВНА"]


def _n(value: str, modulo: int) -> int:
    return int.from_bytes(hashlib.blake2s(SALT + value.encode("utf-8"), digest_size=8).digest(),
                          "big") % modulo
...
def fake_digits(value: str) -> str:
    ...
    if not digits:
        return value
    replacement = str(_n(value, 10 ** len(digits))).rjust(len(digits), "7")[:len(digits)]
    out, i = [], 0
    for ch in value:
        if ch.isdigit():
...
def fake_person(match: re.Match) -> str:
    key = match.group(0)
    return (f"{SURNAMES[_n(key, len(SURNAMES))]} "
...
def scrub_text(text: str) -> str:
    ...
    return _SHORT_NAME.sub(lambda m: SURNAMES[_n(m.group(0), len(SURNAMES))] + " А.А.", text)
...
def anonymize_priorbank(src: str, dst: str) -> None:
    ...
    """
    rows = list(csv.reader(open(src, encoding="cp1251"), delimiter=";"))
    header_at = next((i for i, r in enumerate(rows) if r and r[0].startswith("Дата док")), None)
    head = {n.strip(): i for i, n in enumerate(rows[header_at])} if header_at is not None else {}
    ...
    for i, row in enumerate(rows):
        row = list(row)
        if header_at is not None and i > header_at and len(row) > 4 \
                and re.match(r"^\d{2}\.\d{2}\.\d{4}$", (row[0] or "").strip()):
            for name, fn in (("Корреспондент.Название", fake_company),
...
def main() -> None:
    ...
    wanted = ["Vpsk_BY79PJCB30120097991000000933-2.csv",
