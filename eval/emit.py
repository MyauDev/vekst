...
TEMPLATE_SQL_HEADER = """-- The industry template: 60 categories that belong to whoever adopts them,
...
def write_industry_template_sql():
    """core/migrations/00018's copy of the industry template.

    Reads eval/out/industry_template.csv back rather than recomputing from
    `cats` a second way -- the CSV is already the committed source
    (write_industry_template's own docstring), so this is a straight
    CSV-to-SQL transcription, the same relationship seed_categories.sql has
    to the founder's source files, one step further removed. It has no
    dependency on prepare()'s external source, so it can run -- and is
    re-run by hand -- without ../docCl present.
    """
    with open(
        os.path.join(OUT, "industry_template.csv"), newline="", encoding="utf-8-sig"
    ) as f:
        rows = list(csv.DictReader(f, delimiter=";"))

    with open(os.path.join(OUT, "industry_template.sql"), "w", encoding="utf-8") as f:
        f.write(TEMPLATE_SQL_HEADER)
        lines = [
            f"  ({_sql(TAXONOMY_VERSION)}, {_sql(r['code'])}, {_sql(r['parent_code'])}, "
            f"{r['level']}, {_sql(r['name'])}, {str(bool(r['is_leaf'])).lower()}, "
            f"{str(r['in_pnl'] == 'yes').lower()})"
            for r in rows
        ]
        f.write(",\n".join(lines) + ";\n")
