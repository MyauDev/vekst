...
| 5.3 | `connect-app-end-to-end` — the change this table's own 2026-09-19 status report named as owning nothing: `OrgService.CreateOrganization`, industry-template adoption, `GetCurrentUser` carrying organisations, a first-run screen, and `imports.ts`/`review.ts`/`report.ts` reading `core` instead of fixtures. **Implemented 2026-09-20, migration 018**: `category_templates` and `currencies`, both global and RLS-exempt. Three capabilities (`tenancy`, `identity-access`, `web-app`), argued as one change in its own design §0 rather than split, because no half is independently demonstrable. **Found and corrected during implementation, not anticipated by the proposal**: `ListOrganisationsForUser`/`CountMembershipsForUser` could not be plain sqlc queries (the same RLS chicken-and-egg `orgs_for_user` already exists for — a caller with no organisation yet has no tenant context to query under), so `db.MembershipsForUser` was added beside `OrgIDForSession` instead, the same door answering "which ones" rather than "is it this one"; `AdoptIndustryTemplate` could not live in `core/internal/tenancy` as designed without an import cycle, so it lives in `core/internal/db` beside the transaction it runs in; the fixture `report.ts`'s three-section grouping had no backend equivalent — `core/internal/report/pnl.go`'s own `Order` is twelve flat, interleaved lines, never one row per leaf category — so `PnlTable` needed no section/subtotal logic at all, and gained the required exclusion-bucket rows CLAUDE.md's own invariant asks for and the fixture never rendered. Not yet merged: no PR opened, no `openspec/changes/` archival | `tenancy`, `identity-access`, `web-app` | both | 1.5–2 pre-estimated (STATUS-2026-09-19.md), not re-measured against a single long implementation session |
...
| D-11 | Product name: Vekst or Palm | Anything public | **Closed, decided as "Veekst"** (double `e`, `veekst.com`) in `add-web-experience`'s proposal and shipped in `web/index.html` and `web/src/i18n.ts` since that change merged. This row was never marked closed at the time |
