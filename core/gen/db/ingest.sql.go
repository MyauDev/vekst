...
const abandonExpiredBatch = `-- name: AbandonExpiredBatch :one
...
func (q *Queries) AbandonExpiredBatch(ctx context.Context, id pgtype.UUID) (pgtype.UUID, error) {
	row := q.db.QueryRow(ctx, abandonExpiredBatch, id)
	var id_2 pgtype.UUID
	err := row.Scan(&id_2)
	return id_2, err
}

const findImportedBatchAlreadyHoldingThisFile = `-- name: FindImportedBatchAlreadyHoldingThisFile :one
...
// it.
func (q *Queries) FindImportedBatchAlreadyHoldingThisFile(ctx context.Context, fileSha256 []byte) (pgtype.UUID, error) {
	row := q.db.QueryRow(ctx, findImportedBatchAlreadyHoldingThisFile, fileSha256)
	var id pgtype.UUID
	err := row.Scan(&id)
	return id, err
}
...
type InsertImportBatchRow struct {
	ID                 pgtype.UUID
	OrgID              pgtype.UUID
	EntityID           pgtype.UUID
	SourceKind         string
...
func (q *Queries) InsertImportBatch(ctx context.Context, arg InsertImportBatchParams) (InsertImportBatchRow, error) {
	row := q.db.QueryRow(ctx, insertImportBatch,
		arg.OrgID,
		arg.ID,
		arg.EntityID,
		arg.SourceKind,
...
const insertRawRows = `-- name: InsertRawRows :execrows
...
type InsertRawRowsParams struct {
	OrgID   pgtype.UUID
	BatchID pgtype.UUID
	Column3 []int32
	Column4 []string
...
func (q *Queries) InsertRawRows(ctx context.Context, arg InsertRawRowsParams) (int64, error) {
	result, err := q.db.Exec(ctx, insertRawRows,
		arg.OrgID,
		arg.BatchID,
		arg.Column3,
		arg.Column4,
...
func (q *Queries) ListImportBatches(ctx context.Context, entityID pgtype.UUID) ([]ListImportBatchesRow, error) {
	rows, err := q.db.Query(ctx, listImportBatches, entityID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const recordUploadMeasurement = `-- name: RecordUploadMeasurement :execrows
UPDATE import_batches
...
type RecordUploadMeasurementParams struct {
	ID          pgtype.UUID
	FileSha256  []byte
	ByteLength  pgtype.Int8
	ContentType pgtype.Text
...
func (q *Queries) RecordUploadMeasurement(ctx context.Context, arg RecordUploadMeasurementParams) (int64, error) {
	result, err := q.db.Exec(ctx, recordUploadMeasurement,
		arg.ID,
		arg.FileSha256,
		arg.ByteLength,
		arg.ContentType,
...
func (q *Queries) SetImportBatchStatus(ctx context.Context, arg SetImportBatchStatusParams) (int64, error) {
	result, err := q.db.Exec(ctx, setImportBatchStatus, arg.ID, arg.Status, arg.FailureCode)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
const setResolvedParameters = `-- name: SetResolvedParameters :execrows
UPDATE import_batches
SET resolved_parameters = $2, updated_at = now()
WHERE id = $1
`

type SetResolvedParametersParams struct {
	ID                 pgtype.UUID
	ResolvedParameters []byte
}
...
func (q *Queries) SetResolvedParameters(ctx context.Context, arg SetResolvedParametersParams) (int64, error) {
	result, err := q.db.Exec(ctx, setResolvedParameters, arg.ID, arg.ResolvedParameters)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
